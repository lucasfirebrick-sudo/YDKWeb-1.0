...
class MultiImageGallery {
    ...
    initializeGallery() {
        ...
        // 产品ID验证 - 防止交叉污染
        const productContainer = mainImage.closest('.product-images');
        const productId = productContainer?.getAttribute('data-product-id');

        // 如果图片被锁定（如保温材料页面），不允许修改
        if (mainImage.hasAttribute('data-locked')) {
            console.log(`Image locked for product: ${productId}`);
            return;
        }
